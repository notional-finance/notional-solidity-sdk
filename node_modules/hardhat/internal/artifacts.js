...
class Artifacts {
    ...
    _getArtifactPathsSync() {
        const paths = glob_1.globSync(path.join(this._artifactsPath, "**/*.json"), {
            ignore: [this._buildInfosGlob, this._dbgsGlob],
        });
        return paths.sort();
    }
    ...
    _getAllContractNamesFromFiles(files) {
        return files.map((file) => {
            ...
            return contract_names_1.parseFullyQualifiedName(fqn).contractName;
        });
    }
...
`;
    ...
    _getSimilarContractNames(givenName, names) {
        ...
        for (const name of names) {
            const distance = contract_names_1.findDistance(givenName, name);
