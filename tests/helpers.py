...
from brownie.network.state import Chain
from scripts.config import CurrencyDefaults, nTokenDefaults, GovernanceConfig
...
def execute_proposal(environment, targets, values, calldatas):
    ...
    multisig = environment.get('multisig')
    txn = governor.propose(targets, values, calldatas, {
                                       "from": multisig})
    proposalId = txn.events["ProposalCreated"]["id"]
    chain.mine(1)
    governor.castVote(
        proposalId, True, {"from": multisig})
    chain.mine(GovernanceConfig["governorConfig"]["votingPeriodBlocks"])

    assert governor.state(proposalId) == 4  # success
    delay = governor.getMinDelay()
    governor.queueProposal(proposalId, targets, values, calldatas, {
        "from": multisig})
    chain.mine(1, timestamp=chain.time() + delay)
    txn = governor.executeProposal(
        proposalId, targets, values, calldatas, {
            "from": multisig})
    return txn
